...
public:
    ...
    static Value *genFieldVar(_A_field_ field) {
        ...
        std::string fullName = recordName + "_" + fieldName;
        return NamedValues[S_Symbol((char *)(fullName.data()))];
    }
    ...
    static Value *genAssignStatement(Value *var, Value *assignExp) {
    ...
    static Value *genVarDec(A_field var) {
    ...
    static Function *genRoutineDec(S_symbol name, A_fieldList params, A_simpleTy simplety, Value *subroutine) {
        ...
        Function::Create(FT, Function::ExternalLinkage, S_name(name), TheModule);
        ...
        for (auto &Arg : TheFunction->args()) {
            auto name = Arg.getName().data();
            NamedValues[S_Symbol((char *)name)] = &Arg;
        }
    ...
    static Type *genSimpleType(A_simpleTy simple) {
        switch(simple->kind) {
            case A_sysTy:
                return genSingleType(simple->u.systy);
            case A_singleTy:
    ...
    static Value *genCompoundStatement(std::vector<Value *> statements) {
        //TODO
        return nullptr;
    }
...
std::map<S_symbol, Value *> IR::NamedValues;
std::map<S_symbol, Type *> IR::NamedTypes;
